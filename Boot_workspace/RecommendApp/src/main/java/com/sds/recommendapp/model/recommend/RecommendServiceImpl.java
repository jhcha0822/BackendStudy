...
import com.sds.recommendapp.model.comments.CommentsDocDAO;
import com.sds.recommendapp.model.movie.MovieApiService;
import com.sds.recommendapp.model.movie.MovieDAO;
...
//		}
		...
		// 영화평 List를 Mahout 전용 List로 변환
		// - List -> PreferenceArray
		// - CommentsDoc -> Preference
		...
		// 영화평 리스트에서 긍정 평가한 리스트 추려내기
//		List<MovieDoc> likedMovies = new ArrayList();
		
//		for(int i=0; i<preferences.size(); i++) {
//			Preference p = preferences.get(i);
//			if(p.getValue() > 2.0) {
//				metadataMap.get(p.getItemID());
//			}
//		}
		...
		// 긍정 평가 리스트
		likedMovies.stream().forEach(m -> log.debug("긍정 평가 영화명: "+m.getMovieNm()));		
		
		// 홈페이지의 모든 영화와 긍정 평가한 영화와의 유사도를 체크하여 유사도가 높은 상위 n개의 영화를 추천영화로 등록
		List<MovieDoc> movieList = movieDocDAO.selectAll(null);
		// list -> map
		Map<Long, MovieDoc> candMap = new HashMap();
		
		movieList.stream()
		.forEach(m -> candMap.put((long)m.getMovie_idx(), m));
		
		// 모든 영화에 대한 계산 결과 저장할 map
		Map<Long, Double> calculatedMap = new HashMap();
		for(MovieDoc movieDoc : likedMovies) { 	 								// 긍정 평가한 영화
			for(Map.Entry<Long, MovieDoc> entry : candMap.entrySet()) { 		// 홈페이지의 모든 영화
				MovieDoc candi = entry.getValue();
				double score = calculate(movieDoc, candi);						// 유사도 측정
				calculatedMap.put(entry.getKey() , score);
			}
		}
		log.debug("계산 결과 사이즈 "+calculatedMap.size());
		...
		// 최종 결과를 담을 리스트(컨트롤러에게 전송)
		List<Movie> resultList = new ArrayList();
		
		// List, Set만 stream 생성가능. map은 set으로 변환하여 스트림 생성
		resultList = calculatedMap.entrySet().stream()
				.sorted(Map.Entry.<Long, Double>comparingByValue().reversed())
				.limit(3)
				.map(e -> movieApiService.getMovie(movieDAO.select((int)(long)e.getKey())))
	...
	public double calculate(MovieDoc m1, MovieDoc m2) {
		...
		String[] directors1 = m1.getDirectors();
		String[] directors2 = m2.getDirectors();
		// collection framework이 다양한 메서드 지원으로 유리하다
		...
		String[] genres2 = m2.getGenres();
		// collection framework이 다양한 메서드 지원으로 유리하다
		List<String> listGenres1 = Arrays.asList(genres1);
		List<String> listGenres2 = Arrays.asList(genres2);
		for(String genre : listGenres1) {
			if(listGenres2.contains(genre) ) {
				score += 0.7;
				break;
		...
		String[] nations2 = m2.getNations();
		// collection framework이 다양한 메서드 지원으로 유리하다
		List<String> listNations1 = Arrays.asList(nations1);
		List<String> listNations2 = Arrays.asList(nations2);
		for(String nation : listNations1) {
			if(listNations2.contains(nation) ) {
				score += 0.5;
				break;
